...
$KillProcesses     = $true
$InstallTimeoutSec = 900   # 15 min; installer hung past this gets killed and verified via registry
...
if ($removedAny -and $x64Installs.Count -eq 0) {
    ...
    try {
        ...
        Write-Log "Installing unified Acrobat 64-bit (silent), timeout $($InstallTimeoutSec/60) min..."
        ...
        if ($proc.WaitForExit($InstallTimeoutSec * 1000)) {
            ...
            Write-Log "Installer still running after $($InstallTimeoutSec/60) minutes. Killing hung process tree (PID $($proc.Id)) and verifying via registry..."
            Start-Process taskkill.exe -ArgumentList "/PID $($proc.Id) /T /F" -Wait -WindowStyle Hidden
            Start-Sleep -Seconds 10

            $x64Check = @(Get-AcrobatInstalls $NativeRoot 'x64')
            if ($x64Check.Count -gt 0) {
                Write-Log "64-bit Acrobat is registered despite the hung installer: $(($x64Check | ForEach-Object { "$($_.DisplayName) $($_.DisplayVersion)" }) -join '; ')"
                Write-Log "Treating as success. Hang was likely caused by open Office apps holding PDFMaker files; in-use files will finalize on next reboot."
            }
            else {
                throw "Installation timed out after $($InstallTimeoutSec/60) minutes and 64-bit Acrobat was not found in the registry - likely blocked by open Office apps. Will retry on next remediation cycle."
            }
        }
    }
    catch {
